...
(function(){
    ...
    function move(opt){//{left:1200,top:400}
        opt=opt||{};
        if(!opt.id) return;
        //把默认的值都写到这个里面；
        var defaultOpt={
        ...
        //把需要设置的值，都放到opt中，然后通过opt赋值给defaultOpt;
        for(var attr in opt){
        ...
        if(typeof defaultOpt.effect === 'number'){
            ...
            tmpEffect=defaultOpt.effect.length>=2?zhufengEffect[effect[0]][effect[1]]:zhufengEffect[effect[0]];
        }
        //为Linear公式准备参数；
        var begin={},change={};
        var time=0;
