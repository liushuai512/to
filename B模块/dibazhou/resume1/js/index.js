...
var messageRender = (function () {
    ...
    //创建了一个临时的方法
    function tempFn() {
        var $cur = $messageList.eq(++step);//获取的是jq对象
        $cur.css({
            opacity: 1,
        ...
        if (step === 2) {
            ...
            $cur.on('webkitTransitionEnd', function () {
                if (isTrigger) return;//清除on方法的俩次请求
                isTrigger = true;

                console.log('lo')
                //显示键盘：完成后执行文字打印机
                $messageKeyBoard.css('transform', 'translateY(0)').on('webkitTransitionEnd', textPrint);
                console.log('tf')
            });
    ...
    function bindSubmit() {
        $messageText.html('');
        $messageKeyBoard.off('webkitTransitionEnd', textPrint).css('transform', 'translateY(3.7rem)');
        autoMessage();
    }
...
/*cube*/
/*如果在移动端实现滑动的操作，我们需要阻止IMG和document的默认行为：IMG默认行为是滑动的时候或拖拽图片产生的虚拟图，不是操作当前的元素，document的默认行为是移动端浏览器的所有滑动大部分都是*/
...
var cubeRender = (function () {
    ...
    function startFn(e) {
        ...
        $(this).attr({
            ...
            changeY: 0,
            isMove: false
        });//使用JQ或者ZP存储的自定义属性值都是字符串，即使你写的不是，它也会当作字符串去存储，以后通过ATTR方法获取的结果是字符串
    }
