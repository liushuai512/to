...
~function () {
    ...
    function check(url) {
        return url.indexOf('?') > -1 ? '&' : '?'; //在数组出现的第一次索引,检测是否包含问号，包含的话返回&；
    }
    ...
    function formatData(obj) {
        ...
        str = str.substring(0, str.length - 1);//从索引0开始截取到索引为str.length-1处，不包含m
        return str;
    }
    ...
    function ajax(options) {
        ...
        var _default = {
            ...
            method: 'get',//请求方式
            data: null,//文件主体
            dataType: 'json',//->text、xml... 返回资源文件的格式
            async: true,//异步读取  false同步
            cache: true,//清缓存
            success: null,//成功 执行回调函数
            error: null//失败 执行回调函数
        };

        //2、使用传递进来的参数配置值，把默认值进行替换
